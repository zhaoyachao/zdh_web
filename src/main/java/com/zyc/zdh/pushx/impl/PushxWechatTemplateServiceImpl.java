...
public class PushxWechatTemplateServiceImpl implements PushxWechatTemplateService {
    ...
    private static final String MINIPROGRAM_TEMPLATE_LIST = "/api/v1/pushx/wechatminiprogram/template/list";
    @Override
    ...
    public WechatTemplateResponse getWechatMiniprogramTemplate(String channel, String template_id, String template_type) throws Exception {
        try{
            ...
            wechatTemplateRequest.setSign(sign);
            String json = JsonUtil.formatJsonString(wechatTemplateRequest);
            String response = HttpUtil.builder().retryCount(1).postJSON(ConfigUtil.getValue(ConfigUtil.ZDH_PUSHX_BASE_URL)+MINIPROGRAM_TEMPLATE_LIST, json);
