  ...
  (function() {
      ...
      $('#add').click(function () {
          parent.layer.open({
              ...
              area : ['45%', '60%'],
              //area: ['450px', '500px'],
              content: server_context+"/etl_task_unstructure_add_index?id=-1", //iframe的url
              end : function () {
                  console.info("弹框结束");
      ...
      function deleteMs(ids) {
          $.ajax({
              url : server_context+"/etl_task_unstructure_delete",
              data : "ids=" + ids,
              type : "post",
              dataType : "json",
      ...
      window.operateEvents = {
          ...
          'click #copy': function (e, value, row, index) {
              ...
              top.layer.open({
                  ...
                  area : ['45%', '60%'],
                  //area: ['450px', '500px'],
                  content: server_context+"/etl_task_unstructure_add_index?id="+row.id+"&is_copy=true", //iframe的url
                  end:function () {
                      $('#exampleTableEvents').bootstrapTable('refresh', {
                          url: server_context+"/etl_task_unstructure_list?"+$("#etl_task_unstructure_from").serialize(),
                          contentType: "application/json;charset=utf-8",
      ...
      function operateFormatter(value, row, index) {
          return [
              ...
              '                                    </button>',
              ' <button id="upload" name="upload" type="button" class="btn btn-outline btn-sm" title="上传"><i class="glyphicon glyphicon-file" aria-hidden="true"></i>\n' +
              '                                    </button>',
      ...
      window.operateEvents3 = {
          'click #unstructure_log': function (e, value, row, index) {
              window.open(server_context+"/etl_task_unstructure_log_index.html?unstructure_id=" + row.id);
          }
      };
      ...
      icons: {
        ...
        columns: [{
            ...
            field: 'data_sources_choose_file_input',
            title: '文件读取源',
            sortable:false
        }, {
            field: 'input_path',
          ...
          sortable:false,
            ...
            formatter: function (value, row, index) {
                return [
                    ...
                    '<div class="btn-group">'+
                    '<button type="button" id="unstructure_log" class="btn btn-warning btn-xs">执行记录</button>'+
                    '</div>'+
                    '</div>'
                ].join('');
            }
