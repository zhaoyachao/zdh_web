...
(function(document, window, $) {
  ...
  (function() {
      ...
      function deleteMs(ids) {
          $.ajax({
              url : server_context+"/push_channel_delete",
              data : "ids=" + ids,
              type : "post",
              dataType : "json",
              success : function(data) {
                  ...
                  $('#exampleTableEvents').bootstrapTable('refresh', {
                      url: server_context+"/push_channel_list_by_page?"+$("#push_channel_form").serialize(),
                      contentType: "application/json;charset=utf-8",
                      dataType: "json",
                      queryParams: function (params) {
                          // 此处使用了LayUi组件 是为加载层
      ...
      window.operateEvents = {
          'click #edit': function (e, value, row, index) {
              ...
              top.layer.open({
                  ...
                  area : ['100%', '100%'],
                  //area: ['450px', '500px'],
                  content: server_context+"/push_channel_add_index?id="+row.id, //iframe的url
                  end:function () {
                      $('#exampleTableEvents').bootstrapTable('refresh', {
                          url : server_context+'/push_channel_list_by_page?'+$("#push_channel_form").serialize(),
                          contentType: "application/json;charset=utf-8",
          ...
          'click #copy': function (e, value, row, index) {
              ...
              top.layer.open({
                  ...
                  area : ['100%', '100%'],
                  //area: ['450px', '500px'],
                  content: server_context+"/push_channel_add_index?id="+row.id+"&is_copy=true", //iframe的url
                  end:function () {
                      $('#exampleTableEvents').bootstrapTable('refresh', {
                          url: server_context+"/push_channel_list_by_page?"+$("#push_channel_form").serialize(),
                          contentType: "application/json;charset=utf-8",
          ...
          'click #push_channel_execute': function (e, value, row, index) {
              parent.layer.open({
                  type: 2,
                  title: '手动执行配置',
                  shadeClose: false,
      ...
      function operateFormatter(value, row, index) {
          return [
              ...
              '                                    </button>',
              ' <button id="push_channel_execute" name="push_channel_execute" type="button" class="btn btn-outline btn-sm" title="执行策略"><i class="glyphicon glyphicon-refresh" aria-hidden="true"></i>\n' +
              '                                    </button>'
               +
              '</div>'
      ...
      window.operateEvents2 = {
          'click #status': function (e, value, row, index) {
              ...
              layer.confirm(text, {
                  ...
                  $.ajax({
                      url : server_context+"/push_channel_update_status",
        ...
        columns: [{
            ...
            formatter: function (value, row, index) {
                ...
                return [
                    '<div style="text-align:center" >'+
                    '<div class="btn-group">'+
                    '<button id="status" type="button" class="btn '+class_str+'">'+context+'</button>'+
                    '</div>'+
                    '</div>'
                ].join('');
            }
