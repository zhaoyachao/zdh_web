  ...
  (function() {
      ...
      $('#add').click(function () {
          parent.layer.open({
              ...
              area : ['100%', '100%'],//[宽度,高度]
              //area: ['450px', '500px'],
              content: server_context+"/wechat_media_add_index?id=-1", //iframe的url
              end : function () {
                  console.info("弹框结束");
                  $('#exampleTableEvents-table').bootstrapTable('destroy');
                  $('#exampleTableEvents').bootstrapTable('refresh', {
                      url: server_context+"/wechat_media_list_by_page?"+$("#wechat_media_form").serialize()+"&tm="+new Date(),
      ...
      function deleteMs(ids) {
          $.ajax({
              url : server_context+"/wechat_media_delete",
              data : "ids=" + ids,
              type : "post",
              dataType : "json",
              success : function(data) {
                  ...
                  $('#exampleTableEvents').bootstrapTable('refresh', {
                      url: server_context+"/wechat_media_list_by_page?"+$("#wechat_media_form").serialize(),
                      contentType: "application/json;charset=utf-8",
                      dataType: "json"
                  });
              },
      ...
      window.operateEvents = {
          'click #edit': function (e, value, row, index) {
              ...
              top.layer.open({
                  ...
                  area : ['100%', '100%'],
                  //area: ['450px', '500px'],
                  content: server_context+"/wechat_media_add_index?id="+row.id, //iframe的url
                  end:function () {
                      $('#exampleTableEvents').bootstrapTable('refresh', {
                          url : server_context+'/wechat_media_list_by_page'
                      });
                  }
              });
          ...
          'click #copy': function (e, value, row, index) {
              ...
              top.layer.open({
                  ...
                  area : ['100%', '100%'],
                  //area: ['450px', '500px'],
                  content: server_context+"/wechat_media_add_index?id="+row.id+"&is_copy=true", //iframe的url
                  end:function () {
                      $('#exampleTableEvents').bootstrapTable('refresh', {
                          url: server_context+"/wechat_media_list_by_page?"+$("#wechat_media_form").serialize(),
                          contentType: "application/json;charset=utf-8",
