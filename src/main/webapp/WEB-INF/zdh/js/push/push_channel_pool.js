  ...
  (function() {
      ...
      $('#add').click(function () {
          parent.layer.open({
              ...
              area : ['100%', '100%'],//[宽度,高度]
              //area: ['450px', '500px'],
              content: server_context+"/push_channel_pool_add_index?id=-1", //iframe的url
              end : function () {
                  console.info("弹框结束");
                  $('#exampleTableEvents-table').bootstrapTable('destroy');
                  $('#exampleTableEvents').bootstrapTable('refresh', {
                      url: server_context+"/push_channel_pool_list_by_page?"+$("#push_channel_pool_form").serialize()+"&tm="+new Date(),
      ...
      function deleteMs(ids) {
          $.ajax({
              url : server_context+"/push_channel_pool_delete",
              data : "ids=" + ids,
              type : "post",
              dataType : "json",
              success : function(data) {
                  ...
                  $('#exampleTableEvents').bootstrapTable('refresh', {
                      url: server_context+"/push_channel_pool_list_by_page?"+$("#push_channel_pool_form").serialize(),
                      contentType: "application/json;charset=utf-8",
                      dataType: "json",
                      queryParams: function (params) {
                          // 此处使用了LayUi组件 是为加载层
      ...
      window.operateEvents = {
          'click #edit': function (e, value, row, index) {
              ...
              top.layer.open({
                  ...
                  area : ['100%', '100%'],
                  //area: ['450px', '500px'],
                  content: server_context+"/push_channel_pool_add_index?id="+row.id, //iframe的url
                  end:function () {
                      $('#exampleTableEvents').bootstrapTable('refresh', {
                          url : server_context+'/push_channel_pool_list_by_page?'+$("#push_channel_pool_form").serialize(),
                          contentType: "application/json;charset=utf-8",
          ...
          'click #copy': function (e, value, row, index) {
              ...
              top.layer.open({
                  ...
                  area : ['100%', '100%'],
                  //area: ['450px', '500px'],
                  content: server_context+"/push_channel_pool_add_index?id="+row.id+"&is_copy=true", //iframe的url
                  end:function () {
                      $('#exampleTableEvents').bootstrapTable('refresh', {
                          url: server_context+"/push_channel_pool_list_by_page?"+$("#push_channel_pool_form").serialize(),
                          contentType: "application/json;charset=utf-8",
          ...
          'click #push_channel_pool_execute': function (e, value, row, index) {
              parent.layer.open({
                  type: 2,
                  title: '手动执行配置',
                  shadeClose: false,
                  ...
                  area : ['45%', '60%'],
                  //area: ['450px', '500px'],
                  content: server_context+"/push_channel_pool_task_exe_detail_index?id="+row.id, //iframe的url
                  end : function () {
                      console.info("弹框结束")
                  }
              });
      ...
      window.operateEvents2 = {
          'click #status': function (e, value, row, index) {
              ...
              layer.confirm(text, {
                  ...
                  $.ajax({
                      url : server_context+"/push_channel_pool_update_status",
                      data : {"id": row.id, "status": to_status},
                      type : "post",
                      dataType : "json",
                      success : function(data) {
                          if(data.code != '200'){
