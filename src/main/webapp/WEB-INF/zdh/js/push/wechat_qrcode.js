  ...
  (function() {
      ...
      $('#add').click(function () {
          parent.layer.open({
              ...
              area : ['100%', '100%'],//[宽度,高度]
              //area: ['450px', '500px'],
              content: server_context+"/wechat_qrcode_add_index?id=-1", //iframe的url
              end : function () {
                  console.info("弹框结束");
                  $('#exampleTableEvents-table').bootstrapTable('destroy');
                  $('#exampleTableEvents').bootstrapTable('refresh', {
                      url: server_context+"/wechat_qrcode_list_by_page?"+$("#wechat_qrcode_form").serialize()+"&tm="+new Date(),
      ...
      function deleteMs(ids) {
          $.ajax({
              url : server_context+"/wechat_qrcode_delete",
              data : "ids=" + ids,
              type : "post",
              dataType : "json",
              success : function(data) {
                  ...
                  $('#exampleTableEvents').bootstrapTable('refresh', {
                      url: server_context+"/wechat_qrcode_list_by_page?"+$("#wechat_qrcode_form").serialize(),
                      contentType: "application/json;charset=utf-8",
                      dataType: "json"
                  });
              },
      ...
      function wechat_qrcode_cloud_edit(id) {
          var index_loading = parent.layer.load(1, {
              shade: [0.1,'#fff'] //0.1透明度的白色背景
          });
          $.ajax({
              url : server_context+"/wechat_qrcode_cloud_edit",
              data : {"id": id},
              type : "post",
              dataType : "json",
              success : function(data) {
      ...
      window.operateEvents = {
          'click #edit': function (e, value, row, index) {
              ...
              top.layer.open({
                  ...
                  area : ['100%', '100%'],
                  //area: ['450px', '500px'],
                  content: server_context+"/wechat_qrcode_add_index?id="+row.id, //iframe的url
                  end:function () {
                      $('#exampleTableEvents').bootstrapTable('refresh', {
                          url: server_context+"/wechat_qrcode_list_by_page?"+$("#wechat_qrcode_form").serialize(),
                          contentType: "application/json;charset=utf-8",
          ...
          'click #copy': function (e, value, row, index) {
              ...
              top.layer.open({
                  ...
                  area : ['100%', '100%'],
                  //area: ['450px', '500px'],
                  content: server_context+"/wechat_qrcode_add_index?id="+row.id+"&is_copy=true", //iframe的url
                  end:function () {
                      $('#exampleTableEvents').bootstrapTable('refresh', {
                          url: server_context+"/wechat_qrcode_list_by_page?"+$("#wechat_qrcode_form").serialize(),
                          contentType: "application/json;charset=utf-8",
          ...
          'click #wechat_qrcode_cloud_edit': function (e, value, row, index) {
              layer.confirm('是否同步微信', {
                  btn: ['确定','取消'] //按钮
              }, function(index){
                  wechat_qrcode_cloud_edit(row.id);
                  layer.close(layer.index)
              }, function(){

              });
          },
          'click #wechat_qrcode_custom_edit': function (e, value, row, index) {
              layer.confirm('生成自定义二维码', {
                  btn: ['确定','取消'] //按钮
              }, function(index){
                  $("#id").val(row.id);
                  top.layer.open({
                      ...
                      shade: 0.1,
                      area : ['100%', '100%'],
                      content: server_context+"/wechat_qrcode_custom_index?id="+row.id, //iframe的url
                      end:function () {
                          $('#exampleTableEvents').bootstrapTable('refresh', {
                              url: server_context+"/wechat_qrcode_list_by_page?"+$("#wechat_qrcode_form").serialize(),
                              contentType: "application/json;charset=utf-8",
          ...
          },
          {
          ...
          },
            {
                ...
                formatter: function (value, row, index) {
                    ...
                    return '<img src="data:image/png;base64,'+value+'" style="width: 100px;height: 100px;" />';
                }
            },
            {
                field: 'qrcode_custom_image',
                title: '自定义二维码',
                sortable:false,
                formatter: function (value, row, index) {
                    if(is_empty(value)){
